...
const DateBadge = ({ date }: { date: string }) => (
  <span style={{
    ...
    fontSize: "17px", fontWeight: 700, padding: "2px 10px",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", padding: "20px" }}>
      ...
      <div style={{
        ...
        <div style={{ position: "absolute", top: "10px", left: "16px", background: c("--c-main"), color: "#fff", borderRadius: "12px", padding: "3px 14px", fontSize: "17px", fontWeight: 700 }}>
          {data.issueLabel}
        </div>
        <div style={{ position: "absolute", top: "12px", right: "16px", fontSize: "17px", color: c("--c-text"), opacity: 0.6 }}>
          {data.publishDate}
        </div>
        <h1 style={{ fontSize: "30px", fontWeight: 900, color: c("--c-main"), margin: "8px 0 4px", letterSpacing: "3px" }}>
          {data.title}
        </h1>
        <p style={{ fontSize: "15px", color: c("--c-text"), opacity: 0.6, margin: 0 }}>{data.subtitle}</p>
      </div>
          ...
          {sec.activityReport.items.map((item, i) => (
            <div key={i} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
                ...
                <span style={{ fontSize: "16px", fontWeight: 700, color: c("--c-text") }}>{item.headline}</span>
              </div>
              <PhotoOrPlaceholder photo={photos[i]} hint={item.headline} index={i} style={{ height: "140px", marginBottom: "6px" }} />
              <p style={{ fontSize: "17px", lineHeight: 1.7, color: c("--c-text"), margin: 0 }}>{item.body}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: "12px", padding: "10px 12px", border: `1.5px solid color-mix(in srgb, ${c("--c-sub")} 25%, white)` }}>
            ...
            {sec.nextSchedule.items.map((item, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                <span style={{ background: `color-mix(in srgb, ${c("--c-sub")} 12%, white)`, color: c("--c-sub"), fontSize: "17px", fontWeight: 700, padding: "2px 8px", borderRadius: "8px", whiteSpace: "nowrap" }}>{item.date}</span>
                <span style={{ fontSize: "15px", fontWeight: 600, color: c("--c-text") }}>{item.event}</span>
              </div>
            ))}
          </div>
          ...
          {sec.notices.items.length > 0 && (
            <div style={{ background: "#fff", borderRadius: "12px", padding: "10px 12px", border: `1.5px solid color-mix(in srgb, ${c("--c-accent")} 25%, white)` }}>
              ...
              {sec.notices.items.map((item, i) => (
                <div key={i} style={{ marginBottom: "6px", paddingLeft: "12px", borderLeft: `3px solid ${c("--c-accent")}`, fontSize: "17px", lineHeight: 1.7, color: c("--c-text") }}>
          ...
          {sec.extraBox && sec.extraBox.title && (
            <div style={{ background: `color-mix(in srgb, ${c("--c-sub")} 8%, white)`, borderRadius: "12px", padding: "10px 12px", border: `1.5px dashed ${c("--c-sub")}` }}>
              <span style={{ fontSize: "15px", fontWeight: 700, color: c("--c-sub") }}>{sec.extraBox.title}</span>
              <p style={{ fontSize: "17px", lineHeight: 1.7, color: c("--c-text"), margin: "4px 0 0" }}>{sec.extraBox.body}</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(sec.memberVoices.items.length, 4)}, 1fr)`, gap: "10px" }}>
            {sec.memberVoices.items.map((v, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: "17px", fontWeight: 700, color: c("--c-text"), marginBottom: "4px" }}>{v.name}</div>
                <div style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), opacity: 0.8 }}>「{v.comment}」</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ marginTop: "12px", paddingTop: "10px", display: "flex", justifyContent: "space-between", alignItems: "flex-end", borderTop: `1.5px solid color-mix(in srgb, ${c("--c-main")} 15%, white)` }}>
        <p style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), opacity: 0.6, margin: 0, maxWidth: "65%" }}>{sec.editorNote.body}</p>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: "17px", color: c("--c-text"), opacity: 0.5 }}>{sec.editor.title}</div>
          <div style={{ fontSize: "17px", fontWeight: 700, color: c("--c-main") }}>{sec.editor.name}</div>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", padding: "20px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "14px", position: "relative" }}>
        <div style={{
          ...
          padding: "8px 32px", borderRadius: "0", fontSize: "17px", fontWeight: 700,
          position: "relative",
        ...
        <p style={{ fontSize: "15px", color: c("--c-text"), opacity: 0.5, margin: 0 }}>{data.subtitle}</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginTop: "12px" }}>
        <div style={{ background: "#fff", borderRadius: "12px", padding: "10px", border: `1.5px solid color-mix(in srgb, ${c("--c-sub")} 20%, white)` }}>
          <SectionHeader title={sec.nextSchedule.title} bg={`color-mix(in srgb, ${c("--c-sub")} 12%, white)`} color={c("--c-sub")} />
          {sec.nextSchedule.items.map((item, i) => (
            <div key={i} style={{ fontSize: "17px", color: c("--c-text"), marginBottom: "6px", display: "flex", gap: "6px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: "12px", padding: "10px", border: `1.5px solid color-mix(in srgb, ${c("--c-accent")} 20%, white)` }}>
          <SectionHeader title={sec.notices.title} bg={`color-mix(in srgb, ${c("--c-accent")} 12%, white)`} color={c("--c-accent")} />
          {sec.notices.items.map((item, i) => (
            <div key={i} style={{ fontSize: "17px", color: c("--c-text"), marginBottom: "6px", lineHeight: 1.6 }}>
      ...
      {/* Footer */}
      <div style={{ marginTop: "12px", paddingTop: "8px", fontSize: "15px", color: c("--c-text"), opacity: 0.5, display: "flex", justifyContent: "space-between" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", padding: "18px", background: `linear-gradient(180deg, ${c("--c-bg")}, color-mix(in srgb, ${c("--c-main")} 5%, white))` }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
          <span style={{ fontSize: "17px", opacity: 0.8, background: "rgba(255,255,255,0.2)", borderRadius: "8px", padding: "2px 10px" }}>{data.issueLabel}</span>
          <span style={{ fontSize: "15px", opacity: 0.7 }}>{data.publishDate}</span>
        </div>
        <h1 style={{ fontSize: "28px", fontWeight: 900, margin: "0 0 2px", letterSpacing: "2px" }}>{data.title}</h1>
        <p style={{ fontSize: "17px", opacity: 0.8, margin: 0 }}>{data.subtitle}</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px", minHeight: 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          <div style={{ background: c("--c-main"), color: "#fff", borderRadius: "10px", padding: "6px 10px", fontSize: "15px", fontWeight: 700, textAlign: "center" }}>
          ...
          {sec.activityReport.items.map((item, i) => (
            <div key={i} style={{ background: "#fff", borderRadius: "10px", padding: "8px", border: `1.5px solid color-mix(in srgb, ${c("--c-main")} 20%, white)` }}>
              ...
              <div style={{ fontSize: "15px", fontWeight: 700, color: c("--c-text"), marginBottom: "4px" }}>{item.headline}</div>
              <PhotoOrPlaceholder photo={photos[i]} hint={item.headline} index={i} style={{ height: "100px", marginBottom: "4px" }} />
              <p style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), margin: 0 }}>{item.body}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          <div style={{ background: c("--c-sub"), color: "#fff", borderRadius: "10px", padding: "6px 10px", fontSize: "15px", fontWeight: 700, textAlign: "center" }}>
          ...
          <div style={{ background: "#fff", borderRadius: "10px", padding: "8px", border: `1.5px solid color-mix(in srgb, ${c("--c-sub")} 20%, white)` }}>
            {sec.nextSchedule.items.map((item, i) => (
              <div key={i} style={{ marginBottom: "8px", paddingBottom: "8px", borderBottom: i < sec.nextSchedule.items.length - 1 ? `1px dashed color-mix(in srgb, ${c("--c-sub")} 20%, white)` : "none" }}>
                ...
                <div style={{ fontSize: "17px", color: c("--c-text") }}>{item.event}</div>
              </div>
            ))}
          </div>
          <div style={{ background: c("--c-accent"), color: "#fff", borderRadius: "10px", padding: "6px 10px", fontSize: "15px", fontWeight: 700, textAlign: "center" }}>
            {sec.notices.title}
          </div>
          <div style={{ background: "#fff", borderRadius: "10px", padding: "8px", border: `1.5px solid color-mix(in srgb, ${c("--c-accent")} 20%, white)` }}>
            {sec.notices.items.map((item, i) => (
              <div key={i} style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), marginBottom: "6px" }}>
                {item.headline && <span style={{ fontWeight: 700 }}>{item.headline}　</span>}{item.body}
              </div>
            ))}
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          <div style={{ background: `color-mix(in srgb, ${c("--c-accent")} 80%, ${c("--c-main")})`, color: "#fff", borderRadius: "10px", padding: "6px 10px", fontSize: "15px", fontWeight: 700, textAlign: "center" }}>
            ...
            <div key={i} style={{ background: "#fff", borderRadius: "10px", padding: "8px", border: `1.5px solid color-mix(in srgb, ${c("--c-accent")} 15%, white)` }}>
              <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "4px" }}>
                <div style={{ width: "24px", height: "24px", borderRadius: "50%", background: c("--c-accent"), display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <span style={{ color: "#fff", fontSize: "17px", fontWeight: 700 }}>{v.name.charAt(0)}</span>
                </div>
                <span style={{ fontSize: "17px", fontWeight: 700, color: c("--c-text") }}>{v.name}</span>
              </div>
              <div style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), opacity: 0.8 }}>「{v.comment}」</div>
            </div>
          ))}
      ...
      <div style={{ marginTop: "10px", padding: "8px 12px", background: `color-mix(in srgb, ${c("--c-main")} 8%, white)`, borderRadius: "10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <p style={{ fontSize: "17px", lineHeight: 1.5, color: c("--c-text"), opacity: 0.6, margin: 0, maxWidth: "70%" }}>{sec.editorNote.body}</p>
        <div style={{ fontSize: "17px", fontWeight: 700, color: c("--c-main") }}>{sec.editor.name}</div>
      </div>
    </div>
  );
