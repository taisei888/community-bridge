  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", padding: "24px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "18px" }}>
        <span style={{ fontSize: "15px", color: c("--c-main"), fontWeight: 600, letterSpacing: "2px" }}>{data.issueLabel}　{data.publishDate}</span>
          ...
          <div key={i} style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                <span style={{ background: c("--c-main"), color: "#fff", fontSize: "17px", fontWeight: 700, padding: "2px 10px", borderRadius: "8px" }}>{item.date}</span>
                <span style={{ fontSize: "17px", fontWeight: 700, color: c("--c-text") }}>{item.headline}</span>
              </div>
              <p style={{ fontSize: "15px", lineHeight: 1.8, color: c("--c-text"), margin: 0 }}>{item.body}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom: "14px", padding: "14px 16px", background: `color-mix(in srgb, ${c("--c-sub")} 8%, white)`, borderRadius: "14px" }}>
        ...
        {sec.nextSchedule.items.map((item, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px", fontSize: "16px", color: c("--c-text") }}>
      ...
      {sec.notices.items.length > 0 && (
        <div style={{ marginBottom: "14px", padding: "12px 16px", background: `color-mix(in srgb, ${c("--c-accent")} 6%, white)`, borderRadius: "14px", borderLeft: `4px solid ${c("--c-accent")}` }}>
          <h3 style={{ fontSize: "17px", fontWeight: 700, color: c("--c-accent"), margin: "0 0 8px" }}>{sec.notices.title}</h3>
          {sec.notices.items.map((item, i) => (
            <p key={i} style={{ fontSize: "15px", lineHeight: 1.7, color: c("--c-text"), margin: "0 0 6px" }}>
      ...
      {sec.memberVoices.items.length > 0 && (
        <div style={{ marginBottom: "12px" }}>
          <h3 style={{ fontSize: "17px", fontWeight: 700, color: c("--c-main"), margin: "0 0 8px" }}>{sec.memberVoices.title}</h3>
          <div style={{ display: "flex", gap: "10px" }}>
            {sec.memberVoices.items.map((v, i) => (
              <div key={i} style={{ flex: 1, padding: "10px", background: "#fff", borderRadius: "12px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                <div style={{ fontSize: "16px", fontWeight: 700, color: c("--c-main"), marginBottom: "4px" }}>{v.name}</div>
                <div style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text") }}>「{v.comment}」</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {sec.extraBox && sec.extraBox.title && (
        <div style={{ marginBottom: "12px", padding: "10px 14px", background: `color-mix(in srgb, ${c("--c-sub")} 6%, white)`, borderRadius: "12px", border: `1.5px dashed ${c("--c-sub")}` }}>
          <div style={{ fontSize: "16px", fontWeight: 700, color: c("--c-sub"), marginBottom: "4px" }}>{sec.extraBox.title}</div>
          <p style={{ fontSize: "15px", lineHeight: 1.7, color: c("--c-text"), margin: 0 }}>{sec.extraBox.body}</p>
        </div>
      )}
      ...
      <div style={{ marginTop: "12px", paddingTop: "10px", borderTop: `1.5px solid color-mix(in srgb, ${c("--c-main")} 12%, white)`, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <p style={{ fontSize: "15px", lineHeight: 1.5, color: c("--c-text"), opacity: 0.5, margin: 0, maxWidth: "60%" }}>{sec.editorNote.body}</p>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: "17px", color: c("--c-text"), opacity: 0.4 }}>{sec.editor.title}</div>
          <div style={{ fontSize: "16px", fontWeight: 700, color: c("--c-main") }}>{sec.editor.name}</div>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", padding: "22px 26px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "16px", padding: "12px 18px", background: `linear-gradient(90deg, ${c("--c-main")}, color-mix(in srgb, ${c("--c-main")} 70%, ${c("--c-sub")}))`, borderRadius: "12px", color: "#fff" }}>
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: "17px", opacity: 0.8, margin: "2px 0 0" }}>{data.subtitle}</p>
        </div>
        <div style={{ textAlign: "right", fontSize: "15px", opacity: 0.8 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px", minHeight: 0 }}>
        ...
        <div>
          ...
          {sec.activityReport.items.map((item, i) => (
            <div key={i} style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: "16px", fontWeight: 700, color: c("--c-text"), marginBottom: "2px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            {sec.nextSchedule.items.map((item, i) => (
              <div key={i} style={{
                ...
                borderRadius: "8px", fontSize: "15px", color: c("--c-text"),
              }}>
        ...
        {sec.notices.items.length > 0 && (
          <div>
            <h2 style={{ fontSize: "17px", fontWeight: 800, color: c("--c-accent"), margin: "0 0 8px", paddingLeft: "10px", borderLeft: `4px solid ${c("--c-accent")}` }}>{sec.notices.title}</h2>
            {sec.notices.items.map((item, i) => (
              <div key={i} style={{ padding: "6px 14px", fontSize: "15px", lineHeight: 1.7, color: c("--c-text") }}>
                {item.headline && <span style={{ fontWeight: 700 }}>{item.headline}　</span>}{item.body}
              </div>
            ))}
          </div>
      ...
      {sec.memberVoices.items.length > 0 && (
        <div style={{ marginTop: "10px", padding: "10px 14px", background: `color-mix(in srgb, ${c("--c-accent")} 5%, white)`, borderRadius: "10px" }}>
          <h3 style={{ fontSize: "16px", fontWeight: 700, color: c("--c-accent"), margin: "0 0 6px" }}>{sec.memberVoices.title}</h3>
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            {sec.memberVoices.items.map((v, i) => (
              <span key={i} style={{ fontSize: "17px", color: c("--c-text") }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", padding: "20px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px", justifyContent: "center", marginBottom: "6px" }}>
          ...
          <span style={{ fontSize: "15px", color: c("--c-main"), fontWeight: 600 }}>{data.issueLabel}</span>
          <div style={{ flex: 1, maxWidth: "60px", height: "2px", background: c("--c-main"), opacity: 0.3 }} />
        </div>
        <h1 style={{ fontSize: "26px", fontWeight: 900, color: c("--c-main"), margin: "0 0 4px", letterSpacing: "3px" }}>{data.title}</h1>
        <p style={{ fontSize: "17px", color: c("--c-text"), opacity: 0.5, margin: 0 }}>{data.subtitle}　{data.publishDate}</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateRows: "auto auto 1fr", gap: "12px", minHeight: 0 }}>
        ...
        <div style={{ background: `color-mix(in srgb, ${c("--c-main")} 5%, white)`, borderRadius: "14px", padding: "14px 16px" }}>
          <h2 style={{ fontSize: "17px", fontWeight: 800, color: c("--c-main"), margin: "0 0 10px" }}>{sec.activityReport.title}</h2>
          <div style={{ display: "flex", gap: "12px" }}>
            {sec.activityReport.items.map((item, i) => (
              <div key={i} style={{ flex: 1 }}>
                <PhotoOrPlaceholder photo={photos[i]} hint={item.headline} index={i} style={{ width: "100%", height: "110px", marginBottom: "6px" }} />
                <div style={{ fontSize: "17px", fontWeight: 700, color: c("--c-main"), marginBottom: "2px" }}>{item.date}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
          <div style={{ background: `color-mix(in srgb, ${c("--c-sub")} 6%, white)`, borderRadius: "14px", padding: "12px 14px" }}>
            <h3 style={{ fontSize: "16px", fontWeight: 800, color: c("--c-sub"), margin: "0 0 8px" }}>{sec.nextSchedule.title}</h3>
            {sec.nextSchedule.items.map((item, i) => (
          ...
          <div style={{ background: `color-mix(in srgb, ${c("--c-accent")} 6%, white)`, borderRadius: "14px", padding: "12px 14px" }}>
            <h3 style={{ fontSize: "16px", fontWeight: 800, color: c("--c-accent"), margin: "0 0 8px" }}>{sec.notices.title}</h3>
            {sec.notices.items.map((item, i) => (
              <div key={i} style={{ fontSize: "17px", lineHeight: 1.6, color: c("--c-text"), marginBottom: "6px" }}>
                {item.headline && <span style={{ fontWeight: 700 }}>{item.headline}　</span>}{item.body}
              </div>
            ))}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: sec.extraBox ? "1.5fr 1fr" : "1fr", gap: "12px" }}>
          {sec.memberVoices.items.length > 0 && (
            <div style={{ background: "#fff", borderRadius: "14px", padding: "12px 14px", border: `1.5px solid color-mix(in srgb, ${c("--c-main")} 12%, white)` }}>
              <h3 style={{ fontSize: "16px", fontWeight: 700, color: c("--c-main"), margin: "0 0 8px" }}>{sec.memberVoices.title}</h3>
              {sec.memberVoices.items.map((v, i) => (
                <div key={i} style={{ fontSize: "15px", color: c("--c-text"), marginBottom: "6px" }}>
          ...
          {sec.extraBox && sec.extraBox.title && (
            <div style={{ background: `color-mix(in srgb, ${c("--c-sub")} 5%, white)`, borderRadius: "14px", padding: "12px 14px", border: `1.5px dashed ${c("--c-sub")}` }}>
              <div style={{ fontSize: "15px", fontWeight: 700, color: c("--c-sub"), marginBottom: "4px" }}>{sec.extraBox.title}</div>
              <p style={{ fontSize: "17px", lineHeight: 1.6, color: c("--c-text"), margin: 0 }}>{sec.extraBox.body}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Footer */}
      <div style={{ marginTop: "10px", paddingTop: "8px", borderTop: `1px solid color-mix(in srgb, ${c("--c-main")} 10%, white)`, fontSize: "17px", color: c("--c-text"), opacity: 0.5, display: "flex", justifyContent: "space-between" }}>
        <span>{sec.editorNote.body}</span>
        <span>{sec.editor.name} / {data.clubName}</span>
      </div>
    </div>
