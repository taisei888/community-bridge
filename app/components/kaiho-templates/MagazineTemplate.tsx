  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", padding: "24px 28px" }}>
      ...
      <div style={{ textAlign: "center", borderBottom: `3px double ${c("--c-main")}`, paddingBottom: "12px", marginBottom: "14px" }}>
        <div style={{ fontSize: "15px", color: c("--c-text"), opacity: 0.5, marginBottom: "4px", letterSpacing: "3px" }}>{data.issueLabel}　{data.publishDate}</div>
        <h1 style={{ fontSize: "32px", fontWeight: 900, color: c("--c-main"), margin: "0 0 4px", letterSpacing: "6px" }}>{data.title}</h1>
        <p style={{ fontSize: "15px", color: c("--c-text"), opacity: 0.6, margin: 0, letterSpacing: "2px" }}>{data.subtitle}</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px", minHeight: 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", borderRight: `1px solid color-mix(in srgb, ${c("--c-main")} 15%, white)`, paddingRight: "18px" }}>
          ...
          {sec.activityReport.items.map((item, i) => (
            <div key={i} style={{ marginBottom: "12px" }}>
              <div style={{ fontSize: "16px", fontWeight: 700, color: c("--c-text"), marginBottom: "4px" }}>
              ...
              <p style={{ fontSize: "17px", lineHeight: 1.8, color: c("--c-text"), margin: 0, textAlign: "justify" }}>{item.body}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {/* Schedule */}
          <h2 style={{ fontSize: "17px", fontWeight: 800, color: c("--c-sub"), margin: "0 0 8px", borderBottom: `2px solid ${c("--c-sub")}`, paddingBottom: "4px" }}>
            {sec.nextSchedule.title}
          </h2>
          {sec.nextSchedule.items.map((item, i) => (
            <div key={i} style={{ display: "flex", gap: "8px", marginBottom: "8px", fontSize: "17px", color: c("--c-text") }}>
          ...
          {sec.notices.items.length > 0 && (
            <>
              <h2 style={{ fontSize: "17px", fontWeight: 800, color: c("--c-accent"), margin: "0 0 8px", borderBottom: `2px solid ${c("--c-accent")}`, paddingBottom: "4px" }}>
                {sec.notices.title}
              </h2>
              {sec.notices.items.map((item, i) => (
                <div key={i} style={{ fontSize: "17px", lineHeight: 1.7, color: c("--c-text"), marginBottom: "6px" }}>
          ...
          {sec.memberVoices.items.length > 0 && (
            <>
              <h2 style={{ fontSize: "16px", fontWeight: 800, color: c("--c-main"), margin: "0 0 8px" }}>{sec.memberVoices.title}</h2>
      ...
      {/* Footer */}
      <div style={{ marginTop: "12px", borderTop: `2px solid ${c("--c-main")}`, paddingTop: "8px", display: "flex", justifyContent: "space-between", fontSize: "17px", color: c("--c-text"), opacity: 0.5 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", padding: "20px 24px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "14px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "20px", marginTop: "6px", fontSize: "15px", color: c("--c-text"), opacity: 0.5 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", minHeight: 0 }}>
        ...
        <div style={{ borderRight: `1px solid color-mix(in srgb, ${c("--c-main")} 12%, white)`, paddingRight: "12px" }}>
          <h3 style={{ fontSize: "15px", fontWeight: 800, color: c("--c-main"), margin: "0 0 8px", textTransform: "uppercase", letterSpacing: "1px" }}>{sec.activityReport.title}</h3>
          {sec.activityReport.items.slice(0, 2).map((item, i) => (
            <div key={i} style={{ marginBottom: "10px" }}>
              ...
              <div style={{ fontSize: "17px", fontWeight: 700, color: c("--c-text"), marginBottom: "2px" }}>
                {item.date} {item.headline}
              </div>
              <p style={{ fontSize: "15px", lineHeight: 1.7, color: c("--c-text"), margin: 0, textAlign: "justify" }}>{item.body}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ borderRight: `1px solid color-mix(in srgb, ${c("--c-main")} 12%, white)`, paddingRight: "12px" }}>
          {sec.activityReport.items.slice(2).map((item, i) => (
            <div key={i} style={{ marginBottom: "10px" }}>
              ...
              <p style={{ fontSize: "15px", lineHeight: 1.7, color: c("--c-text"), margin: 0 }}>{item.body}</p>
            </div>
          ))}
          <div style={{ borderTop: `1px solid color-mix(in srgb, ${c("--c-sub")} 20%, white)`, paddingTop: "8px", marginTop: "8px" }}>
            <h3 style={{ fontSize: "15px", fontWeight: 800, color: c("--c-sub"), margin: "0 0 6px" }}>{sec.nextSchedule.title}</h3>
            {sec.nextSchedule.items.map((item, i) => (
        ...
        <div>
          ...
          <div style={{ borderTop: `1px solid color-mix(in srgb, ${c("--c-main")} 12%, white)`, paddingTop: "8px" }}>
            <h3 style={{ fontSize: "15px", fontWeight: 800, color: c("--c-main"), margin: "0 0 6px" }}>{sec.memberVoices.title}</h3>
            {sec.memberVoices.items.map((v, i) => (
              <div key={i} style={{ fontSize: "15px", color: c("--c-text"), marginBottom: "6px" }}>
                <span style={{ fontWeight: 700 }}>{v.name}</span>「{v.comment}」
              </div>
            ))}
          </div>
          {sec.extraBox && sec.extraBox.title && (
            <div style={{ marginTop: "10px", padding: "8px", background: `color-mix(in srgb, ${c("--c-sub")} 6%, white)`, borderRadius: "4px" }}>
              ...
              <p style={{ fontSize: "17px", lineHeight: 1.6, color: c("--c-text"), margin: "3px 0 0" }}>{sec.extraBox.body}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Footer */}
      <div style={{ marginTop: "10px", borderTop: `1px solid ${c("--c-main")}`, paddingTop: "6px", fontSize: "17px", color: c("--c-text"), opacity: 0.5, textAlign: "center" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", padding: "20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: "14px", marginBottom: "14px" }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <div style={{ position: "absolute", bottom: "8px", left: "8px", background: c("--c-main"), color: "#fff", padding: "3px 10px", borderRadius: "3px", fontSize: "15px", fontWeight: 700 }}>
            {sec.activityReport.items[0]?.headline || data.title}
          </div>
        </div>
        {/* Title + meta */}
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ fontSize: "15px", color: c("--c-main"), fontWeight: 700, marginBottom: "4px" }}>{data.issueLabel}</div>
          <h1 style={{ fontSize: "24px", fontWeight: 900, color: c("--c-main"), margin: "0 0 6px", lineHeight: 1.2 }}>{data.title}</h1>
          <p style={{ fontSize: "17px", color: c("--c-text"), opacity: 0.6, margin: "0 0 8px" }}>{data.subtitle}</p>
          <div style={{ fontSize: "15px", color: c("--c-text"), opacity: 0.4 }}>{data.publishDate} / {data.clubName}</div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: "14px", minHeight: 0 }}>
        {/* Left: articles */}
        <div>
          <h2 style={{ fontSize: "16px", fontWeight: 800, color: c("--c-main"), margin: "0 0 8px", borderLeft: `4px solid ${c("--c-main")}`, paddingLeft: "8px" }}>{sec.activityReport.title}</h2>
          {sec.activityReport.items.map((item, i) => (
            <div key={i} style={{ marginBottom: "10px", paddingBottom: "10px", borderBottom: `1px solid color-mix(in srgb, ${c("--c-main")} 10%, white)` }}>
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <div>
                  <div style={{ fontSize: "17px", fontWeight: 700, color: c("--c-text") }}>{item.date} {item.headline}</div>
                  <p style={{ fontSize: "15px", lineHeight: 1.7, color: c("--c-text"), margin: "3px 0 0" }}>{item.body}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          <div style={{ background: `color-mix(in srgb, ${c("--c-sub")} 8%, white)`, padding: "10px", borderRadius: "6px", borderLeft: `3px solid ${c("--c-sub")}` }}>
            <h3 style={{ fontSize: "15px", fontWeight: 700, color: c("--c-sub"), margin: "0 0 6px" }}>{sec.nextSchedule.title}</h3>
            {sec.nextSchedule.items.map((item, i) => (
      ...
      {/* Footer */}
      <div style={{ marginTop: "10px", paddingTop: "6px", borderTop: `1px solid color-mix(in srgb, ${c("--c-main")} 20%, white)`, fontSize: "17px", color: c("--c-text"), opacity: 0.5, display: "flex", justifyContent: "space-between" }}>
        <span>{sec.editorNote.body}</span>
