  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", padding: "18px" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: "15px", opacity: 0.8, marginBottom: "4px" }}>{data.issueLabel}</div>
        <h1 style={{ fontSize: "28px", fontWeight: 900, margin: "0 0 4px", letterSpacing: "3px" }}>{data.title}</h1>
        <p style={{ fontSize: "17px", opacity: 0.8, margin: "0 0 12px" }}>{data.subtitle}</p>
        ...
        {mainEvent && (
          <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: "12px", padding: "12px 18px", display: "inline-block" }}>
            ...
            {mainEvent.description && <div style={{ fontSize: "17px", opacity: 0.8, marginTop: "4px" }}>{mainEvent.description}</div>}
          </div>
        )}
      </div>
      ...
      <div style={{ marginBottom: "12px" }}>
        <h2 style={{ fontSize: "17px", fontWeight: 800, color: c("--c-main"), margin: "0 0 8px", display: "flex", alignItems: "center", gap: "6px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
          {sec.nextSchedule.items.slice(1).map((item, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontSize: "15px", fontWeight: 800, color: c("--c-main") }}>{item.date}</div>
              <div style={{ fontSize: "15px", fontWeight: 600, color: c("--c-text"), marginTop: "2px" }}>{item.event}</div>
              {item.description && <div style={{ fontSize: "15px", color: c("--c-text"), opacity: 0.7, marginTop: "2px" }}>{item.description}</div>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", minHeight: 0 }}>
        <div>
          <h3 style={{ fontSize: "15px", fontWeight: 700, color: c("--c-sub"), margin: "0 0 6px" }}>{sec.activityReport.title}</h3>
          {sec.activityReport.items.map((item, i) => (
            <div key={i} style={{ marginBottom: "8px" }}>
              ...
              <div style={{ fontSize: "15px", fontWeight: 700, color: c("--c-text") }}>{item.date} {item.headline}</div>
              <p style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), margin: "2px 0 0", opacity: 0.8 }}>{item.body}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          {sec.notices.items.length > 0 && (
            <div style={{ background: `color-mix(in srgb, ${c("--c-accent")} 8%, white)`, borderRadius: "10px", padding: "10px" }}>
              <h3 style={{ fontSize: "15px", fontWeight: 700, color: c("--c-accent"), margin: "0 0 6px" }}>{sec.notices.title}</h3>
          ...
          {sec.memberVoices.items.length > 0 && (
            <div style={{ background: "#fff", borderRadius: "10px", padding: "10px", border: `1px solid color-mix(in srgb, ${c("--c-main")} 15%, white)` }}>
              <h3 style={{ fontSize: "17px", fontWeight: 700, color: c("--c-main"), margin: "0 0 6px" }}>{sec.memberVoices.title}</h3>
              {sec.memberVoices.items.map((v, i) => (
                <div key={i} style={{ fontSize: "15px", color: c("--c-text"), marginBottom: "4px" }}>
          ...
          {sec.extraBox && sec.extraBox.title && (
            <div style={{ background: `color-mix(in srgb, ${c("--c-sub")} 8%, white)`, borderRadius: "10px", padding: "8px", border: `1.5px dashed ${c("--c-sub")}` }}>
              <div style={{ fontSize: "15px", fontWeight: 700, color: c("--c-sub") }}>{sec.extraBox.title}</div>
              <p style={{ fontSize: "17px", lineHeight: 1.5, color: c("--c-text"), margin: "3px 0 0" }}>{sec.extraBox.body}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Footer */}
      <div style={{ marginTop: "10px", fontSize: "17px", color: c("--c-text"), opacity: 0.5, textAlign: "center" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", padding: "18px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px", padding: "10px 16px", borderBottom: `2px solid ${c("--c-main")}` }}>
        <div>
          ...
          <p style={{ fontSize: "17px", color: c("--c-text"), opacity: 0.5, margin: "2px 0 0" }}>{data.subtitle}</p>
        </div>
        <div style={{ background: c("--c-main"), color: "#fff", borderRadius: "10px", padding: "8px 14px", textAlign: "center" }}>
          <div style={{ fontSize: "17px", opacity: 0.8 }}>{data.issueLabel}</div>
          <div style={{ fontSize: "15px", fontWeight: 700 }}>{data.publishDate}</div>
        </div>
      </div>

      {/* Calendar-style schedule grid */}
      <div style={{ marginBottom: "12px" }}>
        <h2 style={{ fontSize: "16px", fontWeight: 800, color: c("--c-main"), margin: "0 0 8px" }}>{sec.nextSchedule.title}</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px" }}>
          {sec.nextSchedule.items.map((item, i) => (
            <div key={i} style={{
              ...
              <div style={{ background: c("--c-main"), color: "#fff", padding: "4px 8px", fontSize: "17px", fontWeight: 700, textAlign: "center" }}>
                {item.date}
              </div>
              <div style={{ padding: "8px", textAlign: "center" }}>
                <div style={{ fontSize: "15px", fontWeight: 700, color: c("--c-text") }}>{item.event}</div>
                {item.description && <div style={{ fontSize: "17px", color: c("--c-text"), opacity: 0.6, marginTop: "2px" }}>{item.description}</div>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ marginBottom: "10px" }}>
        <h3 style={{ fontSize: "15px", fontWeight: 800, color: c("--c-sub"), margin: "0 0 6px" }}>{sec.activityReport.title}</h3>
        <div style={{ display: "flex", gap: "10px", overflow: "hidden" }}>
          {sec.activityReport.items.map((item, i) => (
            <div key={i} style={{ flex: 1, background: "#fff", borderRadius: "10px", padding: "8px", border: `1px solid color-mix(in srgb, ${c("--c-sub")} 15%, white)` }}>
              ...
              <div style={{ fontSize: "15px", fontWeight: 700, color: c("--c-sub") }}>{item.date}</div>
              <div style={{ fontSize: "17px", fontWeight: 700, color: c("--c-text") }}>{item.headline}</div>
              <p style={{ fontSize: "17px", lineHeight: 1.5, color: c("--c-text"), margin: "2px 0 0" }}>{item.body}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", minHeight: 0 }}>
        {sec.notices.items.length > 0 && (
          <div style={{ background: `color-mix(in srgb, ${c("--c-accent")} 6%, white)`, borderRadius: "10px", padding: "10px" }}>
            <h3 style={{ fontSize: "17px", fontWeight: 700, color: c("--c-accent"), margin: "0 0 6px" }}>{sec.notices.title}</h3>
            {sec.notices.items.map((item, i) => (
              <div key={i} style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), marginBottom: "4px" }}>
                • {item.headline && <span style={{ fontWeight: 700 }}>{item.headline} </span>}{item.body}
              </div>
        ...
        <div>
          ...
          {sec.extraBox && sec.extraBox.title && (
            <div style={{ background: `color-mix(in srgb, ${c("--c-sub")} 6%, white)`, borderRadius: "8px", padding: "8px" }}>
              <div style={{ fontSize: "15px", fontWeight: 700, color: c("--c-sub") }}>{sec.extraBox.title}</div>
              <p style={{ fontSize: "17px", lineHeight: 1.5, color: c("--c-text"), margin: "2px 0 0" }}>{sec.extraBox.body}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Footer */}
      <div style={{ marginTop: "8px", fontSize: "17px", color: c("--c-text"), opacity: 0.5, textAlign: "center", borderTop: `1px solid color-mix(in srgb, ${c("--c-main")} 15%, white)`, paddingTop: "6px" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", padding: "18px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "14px" }}>
        ...
        <p style={{ fontSize: "17px", color: c("--c-text"), opacity: 0.5, margin: 0 }}>{data.subtitle} ／ {data.issueLabel} ／ {data.publishDate}</p>
      </div>
      ...
      <div style={{ display: "flex", gap: "0", minHeight: 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <h3 style={{ fontSize: "16px", fontWeight: 800, color: c("--c-main"), margin: "0 0 6px" }}>{sec.nextSchedule.title}</h3>
            {sec.nextSchedule.items.map((item, i) => (
              <div key={i} style={{ display: "flex", gap: "8px", marginBottom: "6px", alignItems: "center" }}>
                <span style={{ background: c("--c-main"), color: "#fff", fontSize: "15px", fontWeight: 700, padding: "2px 8px", borderRadius: "6px", whiteSpace: "nowrap" }}>{item.date}</span>
                <span style={{ fontSize: "17px", fontWeight: 600, color: c("--c-text") }}>{item.event}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ position: "relative" }}>
            ...
            <h3 style={{ fontSize: "16px", fontWeight: 800, color: c("--c-sub"), margin: "0 0 6px" }}>{sec.activityReport.title}</h3>
            {sec.activityReport.items.map((item, i) => (
              <div key={i} style={{ display: "flex", gap: "10px", marginBottom: "8px" }}>
                <PhotoOrPlaceholder photo={photos[i]} hint={item.headline} index={i} style={{ width: "80px", height: "60px", flexShrink: 0 }} />
                <div>
                  <div style={{ fontSize: "15px", fontWeight: 700, color: c("--c-sub") }}>{item.date} {item.headline}</div>
                  <p style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), margin: "2px 0 0" }}>{item.body}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {sec.notices.items.length > 0 && (
            <div style={{ position: "relative" }}>
              ...
              <h3 style={{ fontSize: "15px", fontWeight: 800, color: c("--c-accent"), margin: "0 0 6px" }}>{sec.notices.title}</h3>
              {sec.notices.items.map((item, i) => (
                <div key={i} style={{ fontSize: "15px", lineHeight: 1.6, color: c("--c-text"), marginBottom: "4px" }}>
                  {item.headline && <span style={{ fontWeight: 700 }}>{item.headline} </span>}{item.body}
                </div>
          ...
          {sec.memberVoices.items.length > 0 && (
            <div style={{ position: "relative" }}>
              ...
              <h3 style={{ fontSize: "17px", fontWeight: 700, color: c("--c-main"), margin: "0 0 4px" }}>{sec.memberVoices.title}</h3>
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {sec.memberVoices.items.map((v, i) => (
                  <span key={i} style={{ fontSize: "15px", color: c("--c-text"), background: `color-mix(in srgb, ${c("--c-main")} 6%, white)`, padding: "3px 8px", borderRadius: "6px" }}>
                    {v.name}「{v.comment}」
                  </span>
                ))}
              </div>
      ...
      {/* Footer */}
      <div style={{ marginTop: "10px", fontSize: "17px", color: c("--c-text"), opacity: 0.5, textAlign: "center", borderTop: `1px solid color-mix(in srgb, ${c("--c-main")} 15%, white)`, paddingTop: "6px" }}>
        {sec.editorNote.body} ／ {sec.editor.name} ／ {data.clubName}
      </div>
    </div>
  );
