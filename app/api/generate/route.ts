...
紙面充填ルール（最重要）：
- A4縦1ページを必ず情報で埋めてください。空白は絶対に出さないでください。
- 入力が少ない場合は、以下の方法で紙面を埋めてください：
  1. 活動報告の本文を長めに（150〜200字）書く
  2. 来月の予定を3件以上にする（入力にない場合は「詳細は後日お知らせ」等の一般的な予定を追加）
  3. お知らせを2件以上にする（季節の注意喚起、健康づくりの呼びかけ等を追加）
  4. 会員のひとことを2人以上にする（入力にない場合は「会員の皆さま」からの一般的なコメントを追加）
  5. extraBox を必ず追加する（「今月の一言」「季節の健康メモ」「参加のお誘い」等）
  6. 編集後記を120〜180字で書く
- 入力が「会合名と日付だけ」のような最小限の場合でも、上記すべてを生成してA4を埋めること
- 各セクションの文章は、一般的な地域活動・高齢者の健康づくり・季節の話題で自然に膨らませること

文章量の目安：
- 今月の活動報告：1項目につき150〜200字程度（入力が短い場合はさらに膨らませる）
- 来月の予定：最低3件、1項目につき40〜80字程度
- お知らせ：最低2件、1項目につき60〜100字程度
- 会員のひとこと：最低2人、1人あたり40〜70字程度
- extraBox：必ず生成する。80〜120字程度
- 編集後記：120〜180字程度
...
- 短い入力でも、一般的な地域活動の背景や意義を補ってA4全面を埋める
- 入力が最小限（会合名と日付のみ等）でも、すべてのセクションを生成し、紙面を埋めること
- 活動報告 items は最低2件、来月の予定 items は最低3件、お知らせ items は最低2件、会員のひとこと items は最低2人
- extraBox は必ず生成すること（季節の豆知識、健康メモ、参加のお誘い等）
- 高齢者にも読みやすいように、文章はやさしく、文字が大きく表示される前提で整える
